...
class Scan_folder:
    def __init__(self, folder, all_dset_benchmark):
        
        self.all_dset_benchmark  =  all_dset_benchmark
        self.folder              =  folder
        self.log_fname           =  pop1(lfilter(lambda x: x.endswith('.log'),
                                                 listdir_full_files(folder)  ))
        
        self._A_log            =  reader(self.log_fname)
        self.runner_args        =  self.__scan_dict('# Begin_Input_Arguments' , '# End_Input_Arguments')
        self.finished_training  =  len(lfilter(lambda x: x[:19] == '# Finished_Training', self._A_log)) == 1
    ...
    def __find_ml_error(self                       ,
        ...
        all_dset  =  self.all_dset_benchmark
...
class Group_Folders:
    def __init__(self, args):
        ...
        for folder in listdir_full_folders(pjoin( deepdirname(abspath(__file__),3),
                                                 ...
                                                 'nn_training'                    ,
                                                 'genetic_runs'                   )):
            m = Scan_folder(folder, args['all_dset_benchmark'])
...
def main_runner(device):
    ...
    while True:
        ...
        if not queue_argv:
            break
        for Kfold in wished_kfolds:
            os_system(f"{python_path} {cache_abspath}")
            os_system(f"{python_path} {main_file} {' '.join(map(str,new_argv)).replace('PYTHON_TAG_KFOLD',str(Kfold))}")
            os_system(f"{python_path} {cache_abspath}")
